  ...
  async (req, res, next) => {
    try {
      ...
      const [rows] = await pool.execute(
        `SELECT * FROM interest_matrix_rates ORDER BY product_type, credit_score_min`,
      );
...
interestMatrixRouter.get(
  ...
  async (req, res, next) => {
    try {
      await ensureMilestone3Schema();
      const pool = getPool();
      const [rows] = await pool.execute(`SELECT * FROM interest_matrix_rates ORDER BY product_type`);
    ...
    try {
      ...
      await pool.execute(
        `INSERT INTO interest_matrix_rates (
           id, product_type, loan_type, credit_score_min, credit_score_max,
...
interestMatrixRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const missing = CSV_HEADERS.filter((h) => headerIndex[h] === undefined);
      if (missing.length) {
